        ...
        if ':' in IP:  # try parse with IPv6
            ...
            for x in a:
                if len(x) > 4 or len(x) == 0:
                    return 'Neither'
                if not all(c.lower() in '0123456789abcdef' for c in x):
                    return 'Neither'
            return 'IPv6'
        else:  # try parse with IPv4
            a = IP.split('.')
            if len(a) != 4:
                return 'Neither'
            for x in a:
                if x != '0' and x.startswith('0') or not x.isdigit():
                    return 'Neither'
                try:
                    if not (0 <= int(x) <= 255):
                        return 'Neither'
...
if __name__ == '__main__':
    f = Solution().validIPAddress
    assert f("15.16.-0.1") == 'Neither'
    assert f("2001:db8:85a3:0::8a2E:0370:7334") == 'Neither'
    assert f("20EE:FGb8:85a3:0:0:8A2E:0370:7334") == 'Neither'
    assert f('172.16.254.1') == 'IPv4'
    assert f('2001:0db8:85a3:0:0:8A2E:0370:7334') == 'IPv6'
    assert f('256.256.256.256') == 'Neither'
